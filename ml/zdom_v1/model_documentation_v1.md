...
The system runs all 9 TP models simultaneously, scoring every available IC strategy each minute. It dynamically selects the best trade based on expected value, not a fixed model or strategy. This means the system adapts in real time — some minutes it picks IC_45d with TP45, other minutes IC_10d with TP15, depending on which combination has the highest EV at that moment.

### Entry flow (every minute from 10:00 to 14:59)
...
Once in a trade, monitor each minute at **mid price** of the same 4 locked strikes:
- **SL check first**: if debit >= 2× credit → close immediately (loss). SL is checked before TP each bar.
- **TP check**: if debit <= credit × (1 - tp_pct) → close immediately (win)
- **Carry to close**: if neither hits by 15:00 → close at final mid price. Profit if debit < credit, loss otherwise.
- After exit, immediately return to step 1 of entry flow and re-score for the next trade.

### Fill assumptions
- **Entry**: limit order at mid price with 1-minute fill window. This is what the target simulation uses for entry credit.
- **Exit**: mid price assumption. In the walk-forward backtest, a $0.10 exit slippage is added to model realistic fills.
- **Paper trading on Tradier sandbox will validate both assumptions before live deployment.** If paper trading reveals systematic slippage, the slippage parameter can be adjusted.
