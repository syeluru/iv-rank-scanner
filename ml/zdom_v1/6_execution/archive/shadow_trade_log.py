...
def load_models() -> dict:
    ...
    for horizon, fname in HORIZONS.items():
        ...
        if path.exists():
            try:
                loaded[horizon] = load_model(path)
...
def build_features_for_date(target_date: str, feature_cols: list) -> dict:
    ...
    for fname in ["spx_features.parquet", "options_features.parquet",
                  "model_table.parquet"]:
...
def run_score(target_date: str, dry_run: bool = False):
    ...
    if spx_price:
        ...
        spx_f = DATA_DIR / "spx_features.parquet"
