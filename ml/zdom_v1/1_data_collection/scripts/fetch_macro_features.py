...
Pulls and caches:
  ...
  - Gold (via yfinance GC=F — gold futures)
  - DXY (via yfinance ^DXY or DTWEXBGS from FRED)
  - MOVE Index — discontinued on FRED, omitted
...
def add_derived_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Gold trend + Gold/SPX ratio (added later when SPX available)
...
def main():
    ...
    # ── Gold via yfinance (FRED series discontinued) ──
    print(f"  Fetching Gold via yfinance (GC=F)...")
    try:
        import yfinance as yf
        gold = yf.download("GC=F", start=start_date, end=end_date, progress=False)
        if not gold.empty:
            gold_s = gold["Close"].squeeze()
            gold_s.index = pd.to_datetime(gold_s.index).normalize()
            series_dict["gold_fix_am"] = gold_s
            print(f"    Got {len(gold_s)} observations")
        else:
            print(f"    [empty]")
    except Exception as e:
        print(f"    [error] {e}")
