"""
ZDOM V1.2 Walk-Forward Backtest — S3 (Mac Mini).

Full grid: S3 (knapsack optimal) across 5 folds.
Partner script backtest_walkforward_s1s2.py runs S1+S2 on the other machine.

Slippage: $0.20/side ($0.40 RT)

Usage (on Mac Mini):
  git pull
  python3 -m venv ~/venvs/zdom && source ~/venvs/zdom/bin/activate
  pip install xgboost lightgbm scikit-learn pandas pyarrow scipy
  cd /path/to/iv-rank-scanner
  python3 ml/ZDOM/5_analysis/v1_2/scripts/backtest_walkforward_s3.py
"""
...
ZDOM_DIR    = SCRIPT_DIR.parent.parent.parent
...
STRATEGIES_ALL  = [f"IC_{d:02d}d_25w" for d in range(5, 50, 5)]
STRATEGIES_NO5D = [f"IC_{d:02d}d_25w" for d in range(10, 50, 5)]
...
SKIP_RATES = [round(0.05 + i * 0.01, 2) for i in range(31)]
...
def simulate_s3_day(day_pool, cutoffs, skip_rate, portfolio, sl_budget_start):
    trades      = []
    open_pos    = []
    shadow_pos  = None
    sl_reserved = 0.0
        ...
        if real_cands and sl_avail > 0 and bp_avail > 0:
            ...
            for cand in real_cands:
                ...
                pos = OpenPosition(
                    ...
                    qty=qty, max_loss=qty * cand["max_loss_per_ct"], shadow=False)
                open_pos.append(pos)
...
def simulate(candidate_pool, strategy_type, skip_rate, max_sl_pct,
    ...
    for day in sorted(candidate_pool.keys()):
        ...
        trades, portfolio, _ = simulate_s3_day(day_pool, cutoffs, skip_rate, portfolio, daily_sl)
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"  ZDOM V1.2 — Walk-Forward Grid (S3 — Knapsack)")
    ...
    out_file = OUTPUT_BASE / "walkforward_s3_grid.csv"
