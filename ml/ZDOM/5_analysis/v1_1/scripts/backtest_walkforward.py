...
PROJECT_DIR = Path(__file__).resolve().parent.parent.parent  # analysis/scripts -> analysis -> options-model
...
# Exclude IC_05d_25w
STRATEGIES = [f"IC_{d:02d}d_25w" for d in range(10, 50, 5)]
...
EXIT_SLIPS = [0.10]  # $0.10 exit slippage only (total debit above mid, not per-leg)
SKIP_RATES = [round(x / 100, 2) for x in range(20, 41)]  # 20% to 40% in 1% increments
