...
def get_latest_spx_data():
    """Fetch current SPX and VIX data via yfinance."""
    try:
        import yfinance as yf
        spx = yf.download("^GSPC", period="1y", interval="1d", progress=False, auto_adjust=True)
...
def build_live_features(target_date, feature_cols):
    ...
    # ── SPX technical features ──
    spx_file = DATA_DIR / "spx_features.parquet"
    if spx_file.exists():
        spx = pd.read_parquet(spx_file)
        spx["date"] = pd.to_datetime(spx["date"])

        # Find the row for this date (or most recent prior date)
        spx_day = spx[spx["date"] == target_dt]
        if spx_day.empty:
            spx_day = spx[spx["date"] <= target_dt].tail(1)
        if not spx_day.empty:
            # Take first row (daily features are broadcast across all minutes)
            for col in spx_day.columns:
                if col in feature_cols:
                    features[col] = spx_day[col].iloc[0]
