...
Inputs:
  ...
  data/spxw_0dte_greeks.parquet    — EOD Black-Scholes greeks per strike
...
Features:
  ...
  Chain skew:
    REMOVED (Phase 3) — skew_25/50/100 were 100% null (dead features).
    Use IV-surface skew (skew_25d, skew_10d) from src/iv_surface.py instead.

  IC credit approximation (ATM ± 100/150 pt wings):
    ic_credit_100   — approx IC credit entering at ±100/150 strikes
    ic_credit_50    — approx IC credit entering at ±50/100 strikes

  Gamma Exposure (GEX):
    net_gex              — net dealer gamma exposure (calls positive, puts negative)
    net_gex_normalized   — net GEX / spot price (scale-independent)
    ...
    call_wall_distance   — (call_wall - spot) / spot as pct
    put_wall_distance    — (spot - put_wall) / spot as pct
...
# Import GEX calculator from src/ module
sys.path.insert(0, str(PROJECT_DIR))
from src.gex import compute_gex_features
...
def main():
    ...
    # Load EOD greeks for GEX calculation
    greeks_file = DATA_DIR / "spxw_0dte_greeks.parquet"
    has_greeks = greeks_file.exists()
    if has_greeks:
        greeks = pd.read_parquet(greeks_file)
        greeks["date"] = pd.to_datetime(greeks["date"])
        print(f"  EOD greeks: {greeks.shape}")
    else:
        greeks = pd.DataFrame()
        print("  EOD greeks: not found, GEX features will be NaN")
    ...
    for i, date in enumerate(trading_days):
        ...
        # ── Chain skew — REMOVED (Phase 3) ──
        # skew_25/50/100 were 100% null (dead features). The mid prices at
        # 50pt and 100pt OTM were also >90% null. Dropped entirely.
        # IV-surface skew (skew_25d, skew_10d) from src/iv_surface.py
        # provides proper delta-matched skew instead.
        ...
        # ── Gamma Exposure (GEX) ──
        greeks_day = greeks[greeks["date"] == date] if has_greeks else pd.DataFrame()
        oi_day = eod_day[["strike", "right", "open_interest"]]
        gex_feats = compute_gex_features(greeks_day, oi_day, spx_open)
        row.update(gex_feats)
