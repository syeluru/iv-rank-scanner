...
Inputs:
  data/spxw_0dte_intraday_greeks.parquet — delta/IV snapshot plus bid/ask/mid per strike per minute
  data/spx_1min.parquet                  — SPX minute closes for spx_at_entry
...
def build_iron_condor(greeks_now, strategy, underlying):
    """Build an IC using greeks for strike selection and snapshot mids for entry pricing."""
    ...
    price_map = {
        (row["right"], row["strike"]): row["mid"]
        for _, row in greeks_now.iterrows()
    }

    sc_mid = price_map.get(("call", short_call["strike"]))
    sp_mid = price_map.get(("put", short_put["strike"]))
    lc_mid = price_map.get(("call", long_call["strike"]))
    lp_mid = price_map.get(("put", long_put["strike"]))

    if any(pd.isna(x) for x in [sc_mid, sp_mid, lc_mid, lp_mid]):
        return None
...
def simulate_day(day_greeks, day_spx, date):
    """Simulate all IC strategies at every 1-min entry for one trading day."""
    rows = []

    # Pre-index greek snapshot mids: (right, strike) → Series of minute mids indexed by timestamp
    price_index = {}
    ...
    for i, ts in enumerate(filtered_ts):
        # Use the most recent prior snapshot for a decision made at timestamp ts.
        # This keeps target construction causally aligned with live trading,
        # where the 10:00:00 decision should only use information known before 10:00:00.
        if i == 0:
            continue
        prev_ts = filtered_ts[i - 1]

        greeks_now = day_greeks[day_greeks["timestamp"] == prev_ts]
        if len(greeks_now) < 10:
            continue

        # Use the actual SPX 1-minute T-1 close for the decision at ts.
        spx_prev = day_spx[day_spx["datetime"] == prev_ts]
        if spx_prev.empty:
            continue
        spx_at_entry = spx_prev["close"].iloc[0]

        for strategy in STRATEGIES:
            trade = build_iron_condor(greeks_now, strategy, spx_at_entry)
            if trade is None:
                continue
            ...
            rows.append({
                ...
                "spx_at_entry": round(float(spx_at_entry), 2),
...
def main():
    ...
    spx_1m_file = DATA_DIR / "spx_1min.parquet"
    if not spx_1m_file.exists():
        print("ERROR: Missing SPX 1-minute file. Run fetch scripts first.")
        return

    spx_1m = pd.read_parquet(spx_1m_file, columns=["datetime", "close"])
    ...
    for i, date in enumerate(remaining):
        ...
        try:
            ...
            day_spx = spx_1m[spx_1m["datetime"].dt.date == date]
            if len(day_spx) == 0:
                continue

            day_rows = simulate_day(day_greeks, day_spx, date)
