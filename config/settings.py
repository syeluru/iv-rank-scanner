...
class Settings(BaseSettings):
    ...
    BOT_WAKE_TIME: str = "09:30"  # HH:MM ET pre-flight check time (before ZDOM 10:00 start)
    ...
    BOT_MONITOR_PRINT_INTERVAL: int = 30  # Seconds between status prints
    ...
    # ZDOM V1 Integration
    BOT_ZDOM_ENABLED: bool = True          # Use ZDOM V1 scoring (replaces v2-v8 chain)
    BOT_ZDOM_ENTRY_START: str = "10:00"    # 10am ET per ZDOM spec
    BOT_ZDOM_ENTRY_END: str = "14:59"      # Last entry before 3pm close
    BOT_ZDOM_SCAN_INTERVAL: int = 1        # Minutes between scans
    BOT_ZDOM_SKIP_RATE: float = 0.30       # 30% skip rate
    BOT_ZDOM_SL_MULT: float = 2.0          # 2x credit SL (matches model training)
    BOT_ZDOM_SL_CAP: float = 8.0           # Hard $8 SL cap — SL = min($8, 2x credit)
    BOT_ZDOM_QTY_PER_POSITION: int = 0     # Contracts per S2 position (0=auto-size)
    BOT_ZDOM_MAX_POSITIONS: int = 0        # Max S2 positions to open (0=unlimited)
    BOT_ZDOM_MODELS_DIR: str = "ml/zdom_v1/4_models/v1"
    BOT_ZDOM_DATA_DIR: str = "ml/zdom_v1/2_feature_engineering/outputs"
    BOT_ZDOM_HARD_BLOCKERS: bool = True    # Enable hard blocker rules
    BOT_ZDOM_KILL_SWITCH: bool = False     # Manual halt flag
