...
def add_calendar_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def is_thanksgiving_week(d):
        if d.month != 11:
            return 0
        thanksgiving = None
        thursdays = [d.replace(day=day) for day in range(1, 32)
                     if d.replace(day=day).weekday() == 3]
        thanksgiving = thursdays[3] if len(thursdays) >= 4 else None  # 4th Thursday
        if thanksgiving is None:
            return 0
