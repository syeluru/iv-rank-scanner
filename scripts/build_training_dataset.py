...
def compute_intraday_features(
    ...
    """10 intraday SPX features from 5-min candles up to entry_time."""
    defaults = {
        "intraday_rv": 10.0, "move_from_open_pct": 0.0, "orb_range_pct": 0.0,
        "orb_contained": 1.0, "range_exhaustion": 0.5, "high_low_range_pct": 0.0,
        "price_vs_vwap_pct": 0.0, "momentum_30min": 0.0, "trend_slope_norm": 0.0,
        "volume_ratio": 1.0,
    }
    ...
    # VWAP (use volume if available, otherwise skip)
    ...
    if "volume" in df.columns:
        ...
        if vol_sum.iloc[-1] > 0:
            ...
            vwap = (typical * vol).cumsum() / vol_sum
            vwap_val = float(vwap.iloc[-1])
            if vwap_val > 0:
                price_vs_vwap_pct = ((current_price - vwap_val) / vwap_val) * 100
        avg_vol = vol.mean()
        if avg_vol > 0:
            volume_ratio = float(vol.iloc[-1] / avg_vol)
    ...
    return {
        ...
        "volume_ratio": float(min(volume_ratio, 10.0)),
    }
...
def process_day(
    ...
    suffix = f"_{entry_time.hour:02d}00_w{wing_width}"
...
def print_summary(df: pd.DataFrame, features_only: bool):
    ...
    FEATURE_COLS = [
        ...
        "range_exhaustion", "high_low_range_pct", "price_vs_vwap_pct",
        "momentum_30min", "trend_slope_norm", "volume_ratio",
...
def main():
    ...
    entry_time = dtime(args.entry_hour, 0)
    ...
    logger.info(f"  Entry time:   {entry_time.hour}:00 ET")
    ...
    logger.info("\n" + "=" * 65)
    logger.info(f"Step 3: Processing {len(trading_days)} days with {n_workers} workers...")

    # Count already-cached days
    suffix = f"_{entry_time.hour:02d}00_w{wing_width}"
    pre_cached = sum(
        1 for td in trading_days
        if (DAILY_CACHE_DIR / f"{td.isoformat()}{suffix}.json").exists()
    )
    remaining = len(trading_days) - pre_cached
    ...
    def _process_one(td):
        """Worker function for one trading day."""
        try:
            row = process_day(td, entry_time, wing_width, daily_data, args.features_only, use_theta)
            with _progress_lock:
                _progress["done"] += 1
                done = _progress["done"]
                if done % 25 == 0 or done == 1:
                    ...
                    left = len(trading_days) - done - _progress["cached"]
                    eta = left / rate if rate > 0 else 0
                    logger.info(
                        f"  [{done + _progress['cached']:4d}/{len(trading_days)}] "
                        f"{td} ({_progress['errors']} errors, ETA {eta/60:.1f}m)"
                    )
            return row
        except Exception as e:
            with _progress_lock:
    ...
    with ThreadPoolExecutor(max_workers=n_workers) as executor:
        futures = {executor.submit(_process_one, td): td for td in trading_days}
    ...
    logger.info(
        f"\n  Done: {len(results)} days, {errors} errors, "
